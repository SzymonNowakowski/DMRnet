...
cvg_DMRnet <- function(X, y, family = "gaussian", clust.method = 'complete', o = 5, nlambda = 20, lam = 10^(-7), interc = TRUE, nfolds = 10, method = "DMRnet", agressive = FALSE, plateau_resistant_CV = TRUE, maxp = ifelse(family == "gaussian", ceiling(length(y)/2), ceiling(length(y)/4))){
  ...
  if (family == "gaussian"){
    ...
    for (fold in 1:nfolds){
      ...
      if (method == "GLAMER") {
        dmr <- glamer_4lm(Xtr, ytr, clust.method = clust.method, nlambda = nlambda, maxp = ceiling(maxp))
    ...
    #PP rename dmr.fit
    if (method == "GLAMER") {
      dmr.full <- glamer_4lm(X, y, clust.method = clust.method, nlambda = nlambda, maxp = ceiling(maxp))
    ...
    if (plateau_resistant_CV) {
      mdGIC_rel <- (mdGIC - min(mdGIC))/min(mdGIC)
      errGIC_rel <- (errGIC - min(errGIC))/min(errGIC)
      conglomerateGIC<-max(errGIC_rel) * mdGIC_rel +  max(mdGIC_rel) * errGIC_rel
    } else
    ...
    if (family == "binomial"){
      ...
      #PP rename dmr.fit
      if (method == "GLAMER") {
        dmr.full <- glamer_4glm(X, y, clust.method = clust.method, nlambda = nlambda, lam = lam, maxp = maxp)
        cat("full GLAMER calculated\n")
