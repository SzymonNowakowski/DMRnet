...
cat("binomial PDMR single 4-level factor column:\n")
b_PDMR <- DMRnet(X[15], y, family = "binomial", algorithm="PDMR")
...
cat("binomial PDMR:\n")
b_PDMR <- DMRnet(X, y, family = "binomial", algorithm="PDMR")  #fixed by PR#18-20
...
cat("binomial cvg.PDMR:\n")
b_cv.PDMR  <- cv.DMRnet(X, y, family = "binomial", algorithm="PDMR", nlambda=20)
...
cat("gaussian PDMR single 4-level factor column:\n")
PDMR <- DMRnet(X[15], y, family = "gaussian", algorithm="PDMR")
...
cat("gaussian PDMR:\n")
PDMR <- DMRnet(X, y, family = "gaussian", algorithm = "PDMR")  #fixed by PR#18-20
...
cat("gaussian cvg.PDMR:\n")
cv.PDMR  <- cv.DMRnet(X, y, family = "gaussian", algorithm="PDMR", nlambda=20)
