...
runs<-1000
for (model_choice in c( "cv.GLAF", "gic.GLAF")) {
	...
	while (run<=runs) {
	  ...
	  if (model_choice=="gic.GLAF") {
	    ...
	    cat("GIC\n")
	    gic <- gic.DMR(model.1percent, c = 2)   #we are using existing gic calculation which is compatible with GLAF models
	  ...
	  if (model_choice=="gic.GLAF") {
	    ...
	    prediction<- tryCatch(predict.DMR(model.1percent, newx=adult.test.1percent.x, df = gic$df.min, type="class"),
            ...
            #we use a predict from DMR which is compatible with GLAF models
