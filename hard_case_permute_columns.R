...
execute <- function(family) {
  ...
  mod <- DMRnet(X, y, interc=FALSE, family=family)   #SOSnet gets called - no factor columns
  ...
  mod <- DMRnet(X, y, interc=TRUE, family=family)   #SOSnet gets called - no factor columns
  #pred <- predict(mod, newx=X, df=15)
  MSEC <- sum((y - pred)^2)
  cat("Scenario C: ", MSEA, "\n")
  ...
  mod <- DMRnet(X, y, algorithm="glamer", family=family)
  pred <- predict(mod, newx=X, df=3)
  MSEA <- sum((y - pred)^2)
  cat("Scenario A: ", MSEA, "\n")
