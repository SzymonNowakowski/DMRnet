...
Notice how you can pass other parameters (`xlim=c(1, 10), lwd=2`) to change the default behaviour of the `plot()` function.

To examine the coefficients for a particular model in detail, one can use the `coef()` function. For the sake of the example, let's examine the last model from the plot, with `df=10`:
