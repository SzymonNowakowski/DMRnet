...
cv_glamer_cutpoints <- function(X, y, family = "gaussian", clust.method = 'complete', o = 5, nlambda = 20, lam = 10^(-7), interc = TRUE, nfolds = 10, agressive = FALSE, plateau_resistant_CV = TRUE, maxp = ifelse(family == "gaussian", ceiling(length(y)/2), ceiling(length(y)/4))){
  ...
  if (family == "gaussian"){
    ...
    cut_points <- c(0, sqrt(heights[2:length(heights)] * heights_shifted))   #geometric mean, equivalent to arithmetic mean on a logarithmic scale
    for (fold in 1:nfolds){
      ...
      glamer <- glamer_4lm(Xtr, ytr, cut_points, clust.method = clust.method, nlambda = nlambda, maxp = ceiling(maxp))
  ...
  out <- list(df.min = df.min, dmr.fit = glamer.full, cvm = errGIC, foldid = foldid)
  class(out) <- "cv.DMR"
  cat("cv.out\n")
  return(out)
}
